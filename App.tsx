...
import { Calendar, LayoutDashboard, Settings, ChevronLeft, ChevronRight, Plus, Trash2, CheckCircle, Clock, Zap, Cpu, Copy, ArrowLeft, Download, Upload, User, School, Camera, Bell, BellRing, ExternalLink, ListChecks, Info, Sparkles, Edit3, Palette } from 'lucide-react';
  ...
  const handleImport = () => {
    try {
      ...
      if (Array.isArray(parsed)) {
        onImport(parsed);
        alert(`¡Éxito! Se han procesado ${parsed.length} eventos.`);
        navigate('/calendar');
      } else {
...
const MiniMonth: React.FC<{ date: Date, events?: CalendarEvent[], onDayClick?: (d: Date) => void }> = ({ date, events = [], onDayClick }) => {
  ...
  return (
    <div className="text-[10px] w-full bg-white p-4 rounded-3xl border border-slate-100 hover:shadow-md transition-shadow">
        ...
        {days.map(d => {
          ...
          return (
            <div 
              ...
              className={`h-7 flex items-center justify-center cursor-pointer rounded-lg transition-all relative ${isSelected ? 'bg-emerald-500 text-white font-black' : 'hover:bg-slate-100 text-slate-600'}`}
            >
              {format(d, 'd')}
              {hasEv && !isSelected && <div className="absolute bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 bg-emerald-500 rounded-full" />}
            </div>
          );
        })}
      </div>
...
const Dashboard: React.FC<{ events: CalendarEvent[], onToggle: (id: string) => void, onDelete: (id: string) => void }> = ({ events, onToggle, onDelete }) => {
  ...
  const upcoming = useMemo(() => events.filter(e => !e.isDone && e.date >= today).sort((a,b) => a.date.localeCompare(b.date)).slice(0, 10), [events, today]);
  
  return (
    <div className="max-w-5xl mx-auto p-6 md:p-10 space-y-10">
      <div className="bg-slate-900 rounded-[2.5rem] p-10 text-white shadow-2xl relative overflow-hidden">
        <div className="relative z-10">
          <h2 className="text-3xl font-black mb-2 tracking-tight">Panel de Control</h2>
          <p className="text-slate-400 font-bold uppercase tracking-[0.2em] text-[10px] mb-8">Estado actual de la cocina</p>
          <div className="flex gap-4">
            <div className="bg-emerald-500/20 border border-emerald-500/30 px-6 py-4 rounded-2xl backdrop-blur-md flex items-center gap-3">
                ...
                <span className="text-xs font-black uppercase tracking-widest">{events.filter(e => !e.isDone && e.isAutoGenerated).length} Tareas Logísticas</span>
            </div>
            <div className="bg-white/5 border border-white/10 px-6 py-4 rounded-2xl backdrop-blur-md flex items-center gap-3">
                <Clock size={18} className="text-slate-400"/>
                <span className="text-xs font-black uppercase tracking-widest">{events.filter(e => !e.isDone && !e.isAutoGenerated).length} Eventos Docentes</span>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
        <div className="md:col-span-2 space-y-6">
          <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest px-2">Agenda Próxima</h3>
          <div className="grid grid-cols-1 gap-4">
            {upcoming.map(task => (
              <div key={task.id} className="bg-white border border-slate-200 p-6 rounded-3xl shadow-sm flex items-center justify-between group hover:border-emerald-200 transition-all">
                <div className="flex items-center gap-5">
                  <div className={`w-2 h-10 rounded-full ${task.isAutoGenerated ? 'bg-amber-400' : 'bg-slate-900'}`} />
                  <div>
                    <h4 className="font-black text-slate-800 uppercase text-xs tracking-tight">{task.title}</h4>
                    <p className="text-[10px] font-bold text-slate-400 uppercase">{format(parseISO(task.date), "EEEE, d MMMM", { locale: es })}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                    <button onClick={() => window.open(getGoogleCalendarUrl(task), '_blank')} className="p-3 bg-slate-50 text-slate-400 rounded-2xl hover:bg-emerald-50 hover:text-emerald-600 transition-all"><ExternalLink size={16}/></button>
                    <button onClick={() => onToggle(task.id)} className="p-3 bg-slate-900 text-white rounded-2xl shadow-lg hover:scale-105 active:scale-95 transition-all"><CheckCircle size={18}/></button>
                </div>
              </div>
            ))}
            {upcoming.length === 0 && <div className="p-20 text-center bg-white rounded-3xl border-2 border-dashed border-slate-100 text-slate-300 italic">No hay tareas pendientes.</div>}
          </div>
        </div>
        
        <div className="space-y-6">
          <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest px-2">Guía de Colores</h3>
          <div className="bg-white border border-slate-200 rounded-[2.5rem] p-8 space-y-5 shadow-sm">
            <div className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-xl bg-red-600 flex items-center justify-center text-white font-bold">🚨</div>
                <div>
                    <p className="text-[10px] font-black text-slate-800 uppercase leading-none">Hacer Pedido</p>
                    <p className="text-[9px] font-medium text-slate-400">Lunes anterior</p>
                </div>
            </div>
            <div className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-xl bg-orange-500 flex items-center justify-center text-white font-bold">⚠️</div>
                <div>
                    <p className="text-[10px] font-black text-slate-800 uppercase leading-none">Terminar Pedido</p>
                    <p className="text-[9px] font-medium text-slate-400">Viernes (-2 semanas)</p>
                </div>
            </div>
            <div className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-xl bg-purple-400 flex items-center justify-center text-white font-bold">📖</div>
                <div>
                    <p className="text-[10px] font-black text-slate-800 uppercase leading-none">Crear Menú</p>
                    <p className="text-[9px] font-medium text-slate-400">24 días antes</p>
                </div>
            </div>
          </div>
        </div>
      ...
      {viewMode === 'year' ? (
        <div className="flex-1 bg-white/30 rounded-[3rem] p-4 md:p-8 overflow-y-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {academicMonths.map(m => (
            <MiniMonth key={m.toString()} date={m} events={events} onDayClick={(d) => { setSelectedDay(d); setCurrentMonth(d); setViewMode('month'); }} />
          ))}
        </div>
      ) : (
        <div className="flex-1 flex flex-col lg:flex-row gap-8 overflow-hidden">
            ...
            <div className="bg-white border border-slate-200 rounded-[2.5rem] p-8 shadow-sm">
              <div className="grid grid-cols-7 gap-1">
                  ...
                  {eachDayOfInterval({ start: startOfMonth(currentMonth), end: endOfMonth(currentMonth) }).map(day => {
                    ...
                    return (
                      <div key={dStr} onClick={() => setSelectedDay(day)} className={`h-14 flex flex-col items-center justify-center cursor-pointer rounded-2xl transition-all relative ${isSelected ? 'bg-emerald-600 text-white shadow-xl scale-110 z-10' : holiday ? 'bg-red-50 text-red-500' : 'hover:bg-slate-50 text-slate-700'}`}>
                        <span className="text-xs font-black">{format(day, 'd')}</span>
                        {hasEv && !isSelected && <div className="w-1.5 h-1.5 bg-emerald-500 rounded-full mt-1" />}
                      </div>
                    );
                  })}
              </div>
          ...
          <div className="flex-1 bg-white border border-slate-200 rounded-[2.5rem] shadow-sm flex flex-col overflow-hidden">
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-2">
                {dayEvents.map(ev => (
                  <div key={ev.id} className="flex items-center justify-between p-5 hover:bg-slate-50 rounded-3xl transition-all group border border-transparent hover:border-slate-100">
                    <div className="flex items-center gap-5">
                      <div className={`w-1.5 h-12 rounded-full ${categories.find(c => c.id === ev.type)?.color || 'bg-slate-200'}`} />
                      <div>
                        <h4 className="text-sm font-black text-slate-800 uppercase leading-none tracking-tight">{ev.title}</h4>
...
const SettingsView: React.FC<{ 
  ...
  // Fix: Added isFormOpen state to control category form visibility and fix "setIsModalOpen" error
  ...
  const handleSaveCategory = () => {
    ...
    setNewCatLabel('');
    // Fix: Close category form after saving
    setIsFormOpen(false);
  };
    ...
    <div className="max-w-5xl mx-auto p-6 md:p-10 space-y-10 pb-32">
      ...
      <div className="bg-white border border-slate-200 rounded-[3rem] p-10 shadow-sm space-y-8">
        <div className="flex justify-between items-center border-b pb-6">
          ...
          <button 
            // Fix: Replaced non-existent setIsModalOpen with setIsFormOpen
        ...
        {/* Formulario rápido de edición/creación */}
        {/* Fix: Updated visibility condition to use isFormOpen state */}
        {(editingCategory || isFormOpen) && (
          <div className="bg-emerald-50/50 p-8 rounded-[2.5rem] border border-emerald-100 animate-in fade-in slide-in-from-top-2">
            <div className="flex justify-between items-center mb-6">
                ...
                {/* Fix: Clear states and close form on cancel */}
            ...
            if (file) {
              ...
              reader.onload = ev => {
                try {
                  ...
                  if (confirm("¿Importar datos? Esto sobrescribirá la agenda actual.")) {
                    onImport(data);
                  }
                } catch { alert("Archivo de backup no válido"); }
              };
