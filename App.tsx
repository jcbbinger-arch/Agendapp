    ...
    <div className="text-[10px] w-full bg-white p-4 rounded-3xl border border-slate-100 hover:shadow-md transition-shadow">
      ...
      <div className="grid grid-cols-7 gap-1">
        ...
        {days.map(d => {
          ...
          // Sombrear si es evento IES
          const iesColor = iesEvent ? categories.find(c => c.id === iesEvent.type)?.color.replace('bg-', 'bg-') + '/10' : '';

          return (
            <div 
              key={dStr} 
              onClick={() => onDayClick?.(d)} 
              className={`h-7 flex items-center justify-center cursor-pointer rounded-lg transition-all relative 
                ${holiday ? 'text-red-500 font-bold bg-red-50' : ''} 
                ${iesColor ? iesColor : ''}
                ${isSelected ? 'bg-emerald-500 text-white font-black z-10 shadow-sm scale-110' : 'hover:bg-slate-100 text-slate-600'}`}
            >
              {format(d, 'd')}
              {dayEvents.some(e => !e.isIES && !e.isAutoGenerated) && !isSelected && (
                  <div className="absolute bottom-1 left-1/2 -translate-x-1/2 w-0.5 h-0.5 bg-slate-900 rounded-full" />
              )}
            </div>
          );
        })}
  ...
  const importFromAI = (importedEvents: any[]) => {
    importedEvents.forEach(ev => {
        addEvent({
            ...
            date: ev.date,
            notes: ev.notes || '',
            isIES: true // Marcamos como evento oficial del IES
        });
    });
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
        <div className="md:col-span-2 space-y-6">
          ...
          <div className="grid grid-cols-1 gap-4">
            {upcoming.map(task => (
              <div key={task.id} className={`bg-white border p-6 rounded-3xl shadow-sm flex items-center justify-between group hover:border-emerald-200 transition-all ${task.isIES ? 'border-emerald-100 ring-1 ring-emerald-50 ring-inset' : 'border-slate-200'}`}>
                <div className="flex items-center gap-5">
                  <div className={`w-2 h-10 rounded-full ${task.isIES ? 'bg-emerald-500' : task.isAutoGenerated ? 'bg-amber-400' : 'bg-slate-900'}`} />
                  <div>
                    <h4 className="font-black text-slate-800 uppercase text-xs tracking-tight">
                        {task.isIES && <span className="text-[8px] bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full mr-2">OFICIAL IES</span>}
                        {task.title}
                    </h4>
        ...
        <div className="space-y-6">
          <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest px-2">Referencia Visual</h3>
          <div className="bg-white border border-slate-200 rounded-[2.5rem] p-8 space-y-5 shadow-sm">
            <div className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-xl bg-emerald-100 border border-emerald-200 flex items-center justify-center"></div>
                <div>
                    <p className="text-[10px] font-black text-slate-800 uppercase leading-none">Eventos IES</p>
                    <p className="text-[9px] font-medium text-slate-400 italic">Sombreado oficial</p>
                </div>
            </div>
            <div className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-xl bg-red-50 border border-red-100 flex items-center justify-center text-red-500 font-black text-[10px]">15</div>
                <div>
                    <p className="text-[10px] font-black text-slate-800 uppercase leading-none">Festivos</p>
                    <p className="text-[9px] font-medium text-slate-400 italic">No lectivos Murcia</p>
                </div>
            </div>
          </div>
        </div>
            ...
            <div className="bg-white border border-slate-200 rounded-[2.5rem] p-8 shadow-sm">
              <div className="grid grid-cols-7 gap-1">
                  ...
                  {eachDayOfInterval({ start: startOfMonth(currentMonth), end: endOfMonth(currentMonth) }).map(day => {
                    ...
                    // Sombrear el día si tiene un evento oficial
          ...
          <div className="bg-white rounded-[3rem] w-full max-w-lg p-10 shadow-2xl border border-slate-100 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={(e) => { 
                ...
                onAdd({ 
                    ...
                    isIES: false // Los manuales son del profesor
                }); 
                setCustomAvisos([]);
